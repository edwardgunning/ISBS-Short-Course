...
We can compute it for our sample of functional data as follows.

```{r covariance, eval = FALSE}
cov_fd <- var.fd(fdobj1 = final_fd)
```

The result is a `bifd` ("bivariate functional data") objects. We can evaluate it on a two-dimensional grid as follows.

```{r covariance eval, eval = FALSE}
cov_eval <- eval.bifd(sevalarg = t_grid, tevalarg =  t_grid, bifd = cov_fd)
```

It can then be visualised using a surface plot, a contour plot or a filled contour plot.

```{r covariance view, eval = FALSE}
